...
In the following sections, we will often use the term 'independent' when we mean 'not-dependent', i.e., when we only require one of the lighter forms of independence.
...
Instead, {independent} on parameters and methods will be used for absolute independence.
When a type is deeply immutable, {independent} is the default state, and therefore it will be omitted.
We use {independent1} for the lighter forms of independence.
