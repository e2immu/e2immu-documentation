...
Furthermore, we will also add a 4th rule for level 2 immutability of type `T`:

****
*Rule 4*: Abstract methods must be non-modifying.
****

The consequence of these choices is that implementations and extensions of abstract and non-abstract types that do not add new non-private methods, will still have the same container and level 2 immutability properties.
...
Note that if a type `T` obeys all rules of level 2 immutability, it follows that parameters of type `T`, or any super-type of `T`, in abstract methods in type `T`, are non-modifying.
This is because no extension or implementation of `T` will have any modifying methods available to make modifications.
Note that this statement does not apply to `Comparable`, because the type `T` of `other` is typically an implementation or extension of `Comparable`, rather than `Comparable` itself or a super-type.
