...
This topic will return in greater detail in the section <<higher-level-modifications>>.

Finally, note that sealed types (since JDK 17) reject the 'you can always extend' assumptions of Java types.
In this case, all sub-types are known, and visible.

[#abstract-methods]
=== Abstract methods

Because `java.lang.Object` is a level 2 container, trivial extensions are, too:
...
@E2Container
class ImplementsEmpty implements EmptyInterface { }

@E2Container
class ExtendsEmpty extends ImplementsEmpty { }
----
...
****
Unless otherwise explicitly annotated, we will assume that abstract methods, be they in interfaces or abstract classes, are {nm}.
****
...
The consequence of these choices is that implementations and extensions of abstract and non-abstract types will have the opportunity to have the same level 2 immutability properties.
...
Note that independence in the variant of rule 3 is guaranteed when the return value or parameter is of unbound parameter type.
The property _implicitly immutable type_, which includes unbound parameter types but is wider, cannot be reliably computed in the presence of abstract methods.

While it is possible to compute the immutability and container status of interface types, using the rules presented above, it makes more sense to use the annotations as contracts: they save a lot of annotation work on the abstract methods in the interface.
