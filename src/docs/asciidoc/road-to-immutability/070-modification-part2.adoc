...
. When `method` has the annotation {independent} (or the lighter version, {independent1}), `v` cannot link to `a`.
...
. If `A` is {e2immutable}, then `v` cannot link to `b`, because all its constructor parameters are independent.
. When `b` has been marked {independent} or {independent1}, `v` cannot link to `b`.
