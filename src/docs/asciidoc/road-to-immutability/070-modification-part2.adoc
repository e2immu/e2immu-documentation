...
. When `method` has the annotation {notLinked} (or the version for hidden content, {independent}), `v` cannot link to `a`.
...
. When `b` has been marked {notLinked} or {independent}, `v` cannot link to `b`.
